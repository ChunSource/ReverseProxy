...
void Forward::newOffer()
{
	...
	if(!clientList.isEmpty()) //通过判断用户表 判断当期是否网络拥塞状态
		{
			bindForwardData(vector.first(),clientList.first());
			vector.pop_front();
...
void Forward::newConnect()  //有新的客户进来了
{
	...
	if(vector.isEmpty() == false) //判断用于转发的socket池里面有没有可用的，有九绑定在一起，没有就先存起来，等到有的时候再绑定
		{
			bindForwardData(vector.first(),socket);
			vector.pop_front();
		}
	else
		{
...
void Forward::bindForwardData(QTcpSocket *source, QTcpSocket *target)  //绑定两个socket的数据交换
{
	ForwardData *forwardData = new ForwardData(source,target);
	forwardData->forward();
}
